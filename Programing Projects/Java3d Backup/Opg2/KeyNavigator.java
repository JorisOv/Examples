/*
 *	@(#)KeyNavigator.java 1.6 00/02/10 13:15:51
 *
...
public class KeyNavigator {
    ...
    private	Vector3d	fwdAcc2;
    private	Vector3d	bwdAcc2;
    ...
    private	Vector3d	fwdDrag2;
    private	Vector3d	bwdDrag2;
    private	Vector3d	leftDrag;
    private	Vector3d	rightDrag;
    private	Vector3d	leftDrag2;
    private	Vector3d	rightDrag2;
    ...
    public KeyNavigator(TransformGroup targetTG) {
      ...
      fwdAcc2 =     new Vector3d( 0.0, 0.0,-5*mmx);
      bwdAcc2 =     new Vector3d( 0.0, 0.0, 5*mmx);
      ...
      fwdDrag2 =    new Vector3d( 0.0, 0.0, 1.5*mmx);
      bwdDrag2 =    new Vector3d( 0.0, 0.0,-1.5*mmx);
      leftDrag =   new Vector3d( mmx, 0.0, 0.0);
      rightDrag =  new Vector3d(-mmx, 0.0, 0.0);
      leftDrag2 =   new Vector3d( 1.5*mmx, 0.0, 0.0);
      rightDrag2 =  new Vector3d(-1.5*mmx, 0.0, 0.0);
    ...
    public void integrateTransformChanges() {
      ...
      if (pre < 0.0) {
         if (pre + fwdDrag2.z * deltaTime < 0.0)
            a.add(fwdDrag2);
         else
            a.z -= pre/deltaTime;
      } else if (pre > 0.0) {
         if (pre + bwdDrag2.z * deltaTime > 0.0)
	          a.add(bwdDrag2);
         else
	          a.z -= pre/deltaTime;
      }
      ...
      if (pre < 0.0) {
         if (pre + leftDrag2.x * deltaTime < 0.0)
            a.add(leftDrag2);
         else
	          a.x -= pre/deltaTime;
      } else if (pre > 0.0) {
                   if (pre + rightDrag2.x * deltaTime > 0.0)
	                    a.add(rightDrag2);
                   else
	                    a.x -= pre/deltaTime;
      }
    ...
    //System.err.println("keyCode " + keyCode + "  keyChar " + keyChar);
	    ...
	    if (keyEvent.getID() == KeyEvent.KEY_RELEASED) {
	       ...
	       switch (keyCode) {
	       case KeyEvent.VK_ESCAPE: key_state = 0; 
	           //System.out.println("Release-Escape"); break;
	       ...
	       if (keyChar == '+') key_state |=  PLUS_SIGN;
	       switch (keyCode) {
	       case KeyEvent.VK_Z: key_state = 0; break;
	       case KeyEvent.VK_ESCAPE: key_state = 0; 
	           //System.out.println("Press-Escape"); break;
	       case KeyEvent.VK_UP:	key_state |=  UP_ARROW;	   break;
	       case KeyEvent.VK_DOWN:	key_state |=  DOWN_ARROW;  break;
	       case KeyEvent.VK_LEFT:	key_state |=  LEFT_ARROW;  break;
	       case KeyEvent.VK_RIGHT:	key_state |=  RIGHT_ARROW; break;
